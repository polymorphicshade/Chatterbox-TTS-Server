...
|                       | `max_reference_duration_sec`  | integer       | Maximum duration for reference audio. Longer uploads are trimmed to this length.                              | `30`                     |
...
*   **Reference Audio:**
    ...
    *   **Trimming long clips:** An uploaded clip longer than `audio_output.max_reference_duration_sec` (default `30`, the Turbo model's limit) is trimmed to that length on import rather than rejected — the first N seconds are kept and the rest discarded, with a warning saying so. Trimming happens before de-noising and before chaining. Output is 16-bit WAV, so an imported MP3 becomes a `.wav`. Note this applies at upload time: a longer file placed into `reference_audio/` by hand is still rejected when generation requests it.
    ...
    *   **Pitch and speed:** The **Adjust Reference Voice** sliders under the reference selector change the clip before the model hears it — pitch in semitones (-12 to +12) and speed (0.5x to 2.0x). The two are independent: shifting pitch leaves timing alone and changing speed leaves pitch alone, since both go through a phase vocoder rather than plain resampling. The saved reference file is never modified; the adjusted version is cached alongside it and passed to the model in its place. **Play Sample** plays exactly what the model will receive. Sent per request as `reference_pitch` / `reference_speed` on `POST /tts`, and remembered in the UI via `ui_state.last_reference_pitch` / `last_reference_speed`.
    *   **De-noise:** Ticking **De-noise** next to the reference file selector runs each imported clip through [DeepFilterNet](https://github.com/Rikorose/DeepFilterNet), a deep-learning speech enhancer, before anything else happens — so clips are cleaned against their own noise profile and the cleaned audio is what gets chained. Output is always 48kHz 16-bit WAV, so an imported MP3 becomes a `.wav`. The setting is remembered between sessions (`ui_state.last_denoise_enabled`) and applies to new imports only; it never touches files already in `reference_audio/`. The model weights download automatically on first use. If `deepfilternet` is not installed the upload still succeeds untouched, with a warning naming the install command.
*   **Usage:**
...
*   **`GET /preview_reference`**:
    *   Query parameters: `filename` (required), `pitch` (semitones, -12 to 12, default `0`), `speed` (0.5 to 2.0, default `1.0`).
    *   Returns the reference audio as WAV exactly as the model would receive it — duration-capped and with the pitch/speed adjustment applied. Backs the UI's **Play Sample** button, and shares the generation cache, so previewing a setting warms the file the next generation reuses.
